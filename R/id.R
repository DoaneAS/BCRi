...
#' @param result takes a list where each element is a list with common vdj or clone assiggment and contains db and affinity matrix
concat_global <- function(db, aff_mtx, phenotype_var, indVar="ind"){
  aff_master = matrix(0, nrow = n_unq, ncol = n_unq)
...
#' @export
#' @returns data.frame of diversity metrics and phenotype ##list with affinity matrix, db for clone, and data.frame of per-phenotype diversity metrics using Simposon diversity
prepare_global <- function(db, cloneID=NULL, phenotype_var="subset", cell_id=NULL, clone = "clone_id", cdr3=FALSE, useAffinityWeights=TRUE, log_base=exp(1), distanceCutoff=FALSE) {
  model = "spectral"
  method = "vj"
  linkage = c("single", "average", "complete")
  normalize = "len" ## c("len", "none"),
  ...
  if (n_unq <= 2) {
    ...
    ## make master aff matrix, then fill with submatrices
